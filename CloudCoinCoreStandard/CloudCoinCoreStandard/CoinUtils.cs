    ...
    public class CoinUtils
    {
        ...
        public void setAnsToPansIfPassed(bool partial = false)
        {
            ...
            for (int i = 0; (i < 25); i++)
            {
                ...
                else if (pownArray[i] == 'u' && ! (RAIDA.GetInstance().nodes[i].RAIDANodeStatus == NodeStatus.Ready) && partial == false)//Timed out but there server echoed. So it probably changed the PAN just too slow of a response
                {
                    cc.an[i] = pans[i];
                }
                else
